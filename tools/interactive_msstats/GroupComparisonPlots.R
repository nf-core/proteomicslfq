...
groupComparisonPlots <- function(data=data,
    ...
    if (type == "VOLCANOPLOT") {
        ...
        if (!FCcutoff) {  
            data[data$adj.pvalue >= sig, "colgroup"] <- "No-regulation"
            data[data$adj.pvalue < sig & data[, 3] > 0, "colgroup"] <- "Up-regulated"
            data[data$adj.pvalue < sig & data[, 3] < 0, "colgroup"] <- "Down-regulated" 
        }
    
        if (is.numeric(FCcutoff)) {
            data$colgroup <- "No-regulation"
            
            if (colnames(data)[3] == "log2FC") {
                data[data$adj.pvalue < sig & data[, 3] > log2(FCcutoff), "colgroup"] <- "Up-regulated"
                data[data$adj.pvalue < sig & data[, 3] < (-log2(FCcutoff)), "colgroup"] <- "Down-regulated"
            }
            
            if (colnames(data)[3] == "log10FC") {
                data[data$adj.pvalue < sig & data[, 3] > log10(FCcutoff), "colgroup"] <- "Up-regulated"
                data[data$adj.pvalue < sig & data[, 3] < (-log10(FCcutoff)), "colgroup"] <- "Down-regulated"
            }
        }
    
        data$colgroup <- factor(data$colgroup, levels=c("No-regulation", "Down-regulated", "Up-regulated"))
        ...
        for (i in 1:nlevels(data$Label)) {
            ...
            if (colnames(sub)[3] == "log10FC") {
     	    ...
     	    ## add protein name
            if (ProteinName) {
          	    if (length(unique(subtemp$colgroup)) == 1 & any(unique(subtemp$colgroup) == 'No-regulation')) {
                    ...
                    ptemp <- ptemp +
                        geom_text_repel(data=subtemp[subtemp$colgroup != "No-regulation", ],
                                        aes(label=Protein),
                                        size=text.size,
                                        col='black')
                }
            ...
            ## cutoff lines, FDR only
     	    if (!FCcutoff) { 
                ...
                if (logBase.pvalue == 10) {
         	        pfinal <- ptemp +
         	            geom_hline(aes(yintercept=-log10(sig), linetype='Adj p-value cutoff'),
         	                      colour="darkgrey",
         	                      size=0.6) +
