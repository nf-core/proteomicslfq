...
# Create a parser
p <- arg_parser("Perform statistical tests on label-free MS1 feature quantification data with MSstats")

# Add command line arguments
p <- add_argument(p, "csv", help="OpenMS MSstats output csv", type="character")
p <- add_argument(p, "--mzTab", help="OpenMS mzTab output to be annotated", type="character")
...
csv_input <- argv$csv
mzTab_input <- argv$mzTab
contrast_str <- argv$contrasts
control_str <- argv$referenceCondition
out_prefix <- argv$outputPrefix
if (!is.na(mzTab_input))
{
    filename <- basename(mzTab_input)
    mzTab_output <- paste0(out_prefix,"_",filename)
}
